# Backend Java Developer Challenge
...
Maven or Gradle are both accepted. Use whichever setup helps you keep the solution simple, easy to run, and easy to review.

Persistence is required, but the specific technology is flexible. You may use:

- In-memory storage.
- Local file storage.
- H2, SQLite, PostgreSQL, MySQL or similar.
- Hosted free-tier services such as Supabase, Neon, MongoDB Atlas or similar.

Whichever option you choose, keep the data model clear and intentional. Data access must be isolated behind repositories or ports instead of coupling business logic directly to controllers, arrays, SQL queries, or external clients.

We recommend a simple hexagonal or layered architecture, for example:
...
- MapStruct or explicit mapper classes between API DTOs and domain models.
- Docker Compose for local persistence.
- Database migrations with Flyway, Liquibase or similar.
